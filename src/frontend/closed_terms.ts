...
class InstantiationPropagator {
    ...
    private instantiateNamespaceFunction(ns: NamespaceDeclaration, fdecl: NamespaceFunctionDecl, terms: TypeSignature[]) {
        ...
        if(fkey.startsWith("String")) {
            ...
            if(nns !== undefined) {
                ...
                if(fkey.endsWith("startsWithString")) {
                    const startdecl = nns.functions.find((tt) => tt.name === "s_unicoderope_starts_with_unicoderope") as NamespaceFunctionDecl;
                    this.instantiateNamespaceFunction(nns, startdecl, []);
                }
                if(fkey.endsWith("removePrefixString")) {
                    const predecl = nns.functions.find((tt) => tt.name === "s_unicoderope_remove_prefix_unicoderope") as NamespaceFunctionDecl;
                    this.instantiateNamespaceFunction(nns, predecl, []);
                }
            }
        } 
    ...
    private instantiateCreateDirectExpression(exp: CreateDirectExpression) {
        this.instantiateTypeSignature(exp.srctype, this.currentMapping);
        this.instantiateTypeSignature(exp.trgttype, this.currentMapping);
        this.instantiateExpression(exp.exp);
    }

    private instantiateSqlConnectExpression(exp: SqlConnectExpression) {
        this.instantiateTypeSignature(exp.srctype, this.currentMapping);
        this.instantiateTypeSignature(exp.trgttype, this.currentMapping);
        this.instantiateExpression(exp.exp);
    }
    ...
    private instantiateExpression(exp: Expression) {
        ...
        switch (exp.tag) {
            ...
            case ExpressionTag.CreateDirectExpression: {
                this.instantiateCreateDirectExpression(exp as CreateDirectExpression);
                break;
            }
            case ExpressionTag.SqlConnectExpression: {
                this.instantiateSqlConnectExpression(exp as SqlConnectExpression);
                break;
            }
    ...
    private instantiateUnicodeRopeIteratorTypeDecl(pdecl: PendingNominalTypeDecl) {
        this.instantiateInteralSimpleTypeDeclHelper(pdecl, [], undefined);
    }
    ...
    private instantiateNamespaceTypeDecl(ns: NamespaceDeclaration, pdecl: PendingNominalTypeDecl) {
        ...
        if(tt instanceof EnumTypeDecl) {
            ...
            this.instantiateUnicodeRopeIteratorTypeDecl(pdecl);
        }

...
class TypeChecker {
    ...
    private checkCreateDirectExpression(env: TypeEnvironment, exp: CreateDirectExpression): TypeSignature {
        const oksrctype = this.checkTypeSignature(exp.srctype);
        const oktrgttype = this.checkTypeSignature(exp.trgttype);
    ...
    private checkSqlConnectExpression(env: TypeEnvironment, exp: SqlConnectExpression): TypeSignature {
        const oksrctype = this.checkTypeSignature(exp.srctype);
        const oktrgttype = this.checkTypeSignature(exp.trgttype);
    ...
    private checkExpression(env: TypeEnvironment, exp: Expression, typeinfer: TypeInferContext | undefined): TypeSignature {
        switch (exp.tag) {
			...
			case ExpressionTag.SqlConnectExpression: {
				return this.checkSqlConnectExpression(env,exp as SqlConnectExpression);
			}
...
*/
    ...
    private checkUnicodeRopeIteratorTypeDecl(ns: NamespaceDeclaration, tdecl: UnicodeRopeIteratorTypeDecl) {
        this.checkInteralSimpleTypeDeclHelper(ns, tdecl, true);
    }
    ...
    private checkNamespaceTypeDecls(ns: NamespaceDeclaration, tdecl: AbstractNominalTypeDecl[]) {
        for(let i = 0; i < tdecl.length; ++i) {
            ...
            if(tt instanceof EnumTypeDecl) {
                ...
                this.checkUnicodeRopeIteratorTypeDecl(ns, tt);
            }
